...
def load_data(source_name: str, pid: str):
    return Dataset(dfile=source_name, pid_col_name=pid)

def load_data_from_url(url: str, name: str):
    return Dataset.load_url(url, name)

def load_data_from_df(df: DataFrame):
    dataset = Dataset()
    dataset.load_df(df)
    return dataset
