...
class AbstractVariable(object):
    ...
    def create(cls, attributes):
        if VARIABLE_TYPE in attributes.keys():
            ...
            raise ValueError(
                f"Missing type information for variable {attributes[VARIABLE_NAME]}! Type can be {NOMINAL}, {ORDINAL}, {NUMERIC} ({INTERVAL} or {RATIO})."
            )
...
class NominalVariable(AbstractVariable):
    ...
    def assume(self, prop):
        if prop in NOMINAL_DATA_PROPS:
            ...
            raise ValueError(
                f"{prop} is not a data property that is supported. Did you mean one of {NOMINAL_DATA_PROPS}?"
            )
...
class OrdinalVariable(AbstractVariable):
    ...
    def assume(self, prop):
        if prop in ORDINAL_DATA_PROPS:
            ...
            raise ValueError(
                f"{prop} is not a data property that is supported. Did you mean one of {ORDINAL_DATA_PROPS}?"
            )
...
class NumericVariable(AbstractVariable):
    ...
    def assume(self, prop):
        if prop in NUMERIC_DATA_PROPS:
            ...
            raise ValueError(
                f"{prop} is not a data property that is supported. Did you mean one of {NUMERIC_DATA_PROPS}?"
            )
