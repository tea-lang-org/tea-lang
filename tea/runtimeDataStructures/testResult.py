...
class TestResult(Value):
    ...
    def get_null_hypothesis(self):
        ...
        if self.name in __two_group_tests__:
            ...
            return __stats_tests_to_null_hypotheses__[self.name].format(var1, var2)
            ...
            return __stats_tests_to_null_hypotheses__[self.name].format(
                __test_to_statistic_of_interest__[self.name],
                f"{self.x.metadata[name]} = {cats[0][0]}",
                f"{self.x.metadata[name]} = {cats[1][0]}",
                self.y.metadata[name],
            )
        elif self.name in __many_groups_test__:
            assert self.x
            assert len(self.x.metadata[categories]) > 0

            return __stats_tests_to_null_hypotheses__[self.name].format(self._get_all_groups(), " ")
        elif self.name in __many_groups_outcome_tests__:
            assert self.x
            assert self.y

            return __stats_tests_to_null_hypotheses__[self.name].format(
                __test_to_statistic_of_interest__[self.name], self._get_all_groups(), self.y.metadata[name]
            )
        elif self.name in __categorical_tests__:
            assert self.x
            assert self.y
            var1, var2 = self._calculate_two_group_vars()
            return __stats_tests_to_null_hypotheses__[self.name].format(var1, var2, self.y.metadata[name])
        return ""
    ...
    def set_interpretation(self):
        ...
        if self.test_statistic > 0:
            ...
            assert (
                False
            ), "test_statistic = 0 and it's not a one-sided test. Not sure under what conditions this is possible."

        # TODO Maybe the "means" part could be replacable....
        self.interpretation = (
            f"t({self.dof}) = {'%.5f'%(self.test_statistic)}, p = {'%.5f'%(self.adjusted_p_value)}. "
            if self.dof
            else ""
        )
        if ttest_result == ttest_result.not_significant:
            ...
            if self.name in __two_group_tests__:
                ...
                self.interpretation += (
                    f"The difference in {stat}s of {self.y.metadata[name]} for {self.x.metadata[name]} = {var1} "
                    f"and {self.x.metadata[name]} = {var2} is significant. "
                )
            elif self.name in __many_groups_test__:
                self.interpretation += __stats_tests_to_null_hypotheses__[self.name].format(
                    self._get_all_groups(), " not "
                )
            elif self.name in __many_groups_outcome_tests__:
                stat = __test_to_statistic_of_interest__[self.name]
            ...
            var1, var2 = self._calculate_two_group_outcome_vars()
            self.interpretation += (
                f"The {stat} of {self.y.metadata[name]} for {self.x.metadata[name]} = {var1} is significantly"
    ...
    def add_effect_size_to_interpretation(self):
        ...
        self.interpretation += (
            f"The effect size is {effect_sizes}. "
    ...
    def _get_all_groups(self):
        assert self.x
        return (
            self.x.metadata[name]
            + " = "
            + ", ".join([str(cat[0]) for cat in list(self.x.metadata[categories].items())])
        )

    def _is_one_sided(self):
        return (
            True
            if self.name in __two_group_outcome_tests__
            and (isinstance(self.prediction, GreaterThan) or isinstance(self.prediction, LessThan))
            else False
        )
