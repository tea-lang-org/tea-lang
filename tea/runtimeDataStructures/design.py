...
class AbstractDesign(object):
    ...
    def which_role(self, variable):
        if self.has_x(variable):
            ...
            raise ValueError(
                f"Variable {AbstractVariable.get_name(variable)} does not have a role in this design: {self}"
            )
    ...
    def __init__(self, attributes, variables):
        ...
        for key, value in attributes.items():
            if key in OBS_X:
                ...
                raise ValueError(
                    f"{key} is not available in an {OBS_STUDY}. Did you mean to say the study is an {EXPERIMENT} or that the variable/s are one of {OBS_X}?"
                )
            elif key in EXP_Y:
                raise ValueError(
                    f"{key} is not available in an {OBS_STUDY}. Did you mean to say the study is an {EXPERIMENT} or that the variable/s are one of {OBS_Y}?"
                )
            else:
    ...
    def __init__(self, attributes, variables):
        ...
        for key, value in attributes.items():
            if key in EXP_X:
                ...
                raise ValueError(
                    f"{key} is not available in an {EXPERIMENT}. Did you mean to say the study is an {OBS_STUDY} or that the variable/s are one of {EXP_X}?"
                )
            elif key in OBS_Y:
                raise ValueError(
                    f"{key} is not available in an {EXPERIMENT}. Did you mean to say the study is an {OBS_STUDY} or that the variable/s are one of {EXP_Y}?"
                )
            else:
                print(f"Extra aspects of study design are not necessary and not considered:{key}, {value}")
