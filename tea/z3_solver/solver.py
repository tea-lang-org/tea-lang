...
class Property:
    ...
    def __call__(self, *var_names):
        if len(var_names) != self.arity:
            raise Exception(f"{self.name} property has arity {self.arity} " f"found {len(var_names)} arguments")
    ...
    if len(xs) == 1:
        if len(ys) == 1:
            ...
            if x.is_categorical() and y.is_categorical():
                ...
                for xc in x_cat:
                    for yc in y_cat:
                        data = dataset.select(
                            y.metadata[name], where=[f"{x.metadata[name]} == '{xc}'", f"{y.metadata[name]} == '{yc}'"]
                        )
        ...
        if x0.is_categorical() and x1.is_categorical():
            ...
            for x0c in x0_cat:
                for x1c in x1_cat:
                    data = dataset.select(
                        x1.metadata[name], where=[f"{x.metadata[name]} == '{xc}'", f"{x1.metadata[name]} == '{x1c}'"]
                    )
...
multivariate = Property(
    "is_multivariate", "More than two variables involved in analysis", is_multivariate
)  # May not need this!
...
independent_obs = Property(
    "has_independent_observations", "Independent (not paired) observations", has_independent_observations
)
...
two_or_more_categories = Property(
    "has_two_or_more_categories", "Variable has two or more categories", has_two_or_more_categories
)
three_or_more_categories = Property(
    "has_three_or_more_categories", "Variable has three or more categories", has_three_or_more_categories
)
...
continuous_or_ordinal = Property(
    "is_continuous_or_ordinal", "Continuous OR ORDINAL (not nominal) data", is_continuous_or_ordinal_var
)
...
def construct_axioms(variables):  # List[StatVar]
    ...
    for var in variables:
        ...
        _axioms.append(
            z3.And(
                z3.Or(continuous(var).__z3__, categorical(var).__z3__),
...
def construct_factorial_ANOVA(combined_data: CombinedData):
    ...
    factorial_ANOVA = StatisticalTest(
        "factorial_ANOVA",
        all_vars,  # Variable number of factors
        test_properties=[one_y_variable],
...
def construct_bivariate_tests(combined_data: CombinedData):
    ...
    if len(combined_data.vars) == 2:
        ...
        pearson_corr = StatisticalTest(
            "pearson_corr",
            [x0, x1],
            test_properties=[bivariate],
            properties_for_vars={continuous: [[x0], [x1]], normal: [[x0], [x1]]},
        )

        kendalltau_corr = StatisticalTest(
            "kendalltau_corr",
            [x0, x1],
            test_properties=[bivariate],
            properties_for_vars={continuous_or_ordinal: [[x0], [x1]]},
        )

        spearman_corr = StatisticalTest(
            "spearman_corr",
            [x0, x1],
            test_properties=[bivariate],
            properties_for_vars={continuous_or_ordinal: [[x0], [x1]]},
        ...
        pointbiserial_corr_a = StatisticalTest(
            "pointbiserial_corr_a",
            [x0, x1],
            test_properties=[bivariate, independent_obs],
            properties_for_vars={
                continuous: [[x1]],
                # normal: [[x1]],
                groups_normal: [[x0, x1]],
        ...
        pointbiserial_corr_b = StatisticalTest(
            "pointbiserial_corr_b",
            [x0, x1],
            test_properties=[bivariate, independent_obs],
            properties_for_vars={
                continuous: [[x0]],
        ...
        students_t = StatisticalTest(
            "students_t",
            [x, y],
            test_properties=[bivariate, one_x_variable, one_y_variable, independent_obs],
            properties_for_vars={
                categorical: [[x]],
                two_categories: [[x]],
                continuous: [[y]],
        ...
        mannwhitney_u = StatisticalTest(
            "mannwhitney_u",
            [x, y],
            test_properties=[one_x_variable, one_y_variable, independent_obs],
            properties_for_vars={
                categorical: [[x]],
                two_categories: [[x]],
        ...
        paired_students_t = StatisticalTest(
            "paired_students_t",
            [x, y],
            test_properties=[bivariate, one_x_variable, one_y_variable, paired_obs],
            properties_for_vars={
                categorical: [[x]],
                two_categories: [[x]],
                continuous: [[y]],
        ...
        wilcoxon_signed_rank = StatisticalTest(
            "wilcoxon_signed_rank",
            [x, y],
            test_properties=[bivariate, one_x_variable, one_y_variable, paired_obs],
            properties_for_vars={categorical: [[x]], two_categories: [[x]], continuous: [[y]],},
        )
        ...
        chi_square = StatisticalTest(
            "chi_square",
            [x, y],
            test_properties=[bivariate, independent_obs, greater_than_5_freq],
            properties_for_vars={categorical: [[x], [y]], two_or_more_categories: [[x], [y]]},
        )
        ...
        fishers_exact = StatisticalTest(
            ...
            [x, y],
            test_properties=[bivariate, independent_obs],
            properties_for_vars={categorical: [[x], [y]], two_categories: [[x], [y]]},
        )

        # ANOVAs
        f_test = StatisticalTest(
            "f_test",
            [x, y],  # Variable number of factors
            test_properties=[independent_obs, one_x_variable, one_y_variable],
            properties_for_vars={
                continuous: [[y]],
        ...
        kruskall_wallis = StatisticalTest(
            "kruskall_wallis",
            [x, y],  # Variable number of factors
            test_properties=[independent_obs, one_x_variable, one_y_variable],
            properties_for_vars={
        ...
        rm_one_way_anova = StatisticalTest(
            "rm_one_way_anova",
            [x, y],  # Variable number of factors
            test_properties=[paired_obs, one_x_variable, one_y_variable],
            properties_for_vars={
                continuous: [[y]],
                categorical: [[x]],  # Variable number of factors
                two_or_more_categories: [[x]],
                            ...
                            if MODE == "strict":
                                ...
                                if val:
                                    tea_logger.log_debug(
                                        f"User asserted property: {prop.name} is supported by statistical checking. Tea agrees with the user."
                                    )
                                else:
                                    tea_logger.log_debug(
                                        f"User asserted property: {prop.name}, but is NOT supported by statistical checking. Tea will override user assertion."
                                    )
                                ...
                                if val:
                                    ...
                                    tea_logger.log_debug(
                                        f"User asserted property: {prop.name}, but is NOT supported by statistical checking. User assertion will be considered true."
                                    )
